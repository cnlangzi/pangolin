...
pub async fn handle(
    ...
    let res: Response<Full<Bytes>> = match path {
        ...
        "tun/new" if method == "POST" => routes::tun::handle_create(&app, &merged_params, &csrf_token).await?,
        "tun/edit" if method == "GET" => routes::tun::render_edit_page(&app, query_param_opt(&merged_params, "name"), &csrf_token).await?,
        "tun/edit" if method == "POST" => routes::tun::handle_update(&app, query_param_opt(&merged_params, "name"), &merged_params, &csrf_token).await?,
        "tun/delete" if method == "POST" => routes::tun::handle_delete(&app, query_param_opt(&merged_params, "name"), &csrf_token).await?,
